...
export class DeleteIngredientController implements DeleteIngredientInterface{
  ...
  public constructor(deleteIngredientUseCase: DeleteIngredientUseCaseInterface){
  ...
  public async execute(httpRequest: HttpRequest): Promise<HttpResponse<Ingredient>> {
    try {
      ...
      const deletedIngredient = await this.deleteIngredientUseCase.execute(ingredientId, restaurantId)
