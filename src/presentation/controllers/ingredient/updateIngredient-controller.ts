...
export class UpdateIngredientController implements UpdateIngredientInterface{
  ...
  public constructor(updateIngredientUseCase: UpdateIngredientUseCaseInterface){
  ...
  public async execute(httpRequest: HttpRequest): Promise<HttpResponse<Ingredient>> {
    try {
      ...
      const updatedIngredient = await this.updateIngredientUseCase.execute(updateIngredientDto)
