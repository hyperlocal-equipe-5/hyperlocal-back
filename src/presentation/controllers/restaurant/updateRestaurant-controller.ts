...
export class UpdateRestaurantController implements UpdateRestaurantInterface{
  ...
  public constructor(updateRestaurantUseCase: UpdateRestaurantUseCaseInterface){
  ...
  public async execute(httpRequest: HttpRequest): Promise<HttpResponse<Restaurant>> {
    try {
      ...
      const updatedRestaurant = await this.updateRestaurantUseCase.execute(updateRestaurantDto)
